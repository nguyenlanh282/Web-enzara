...
export default function AdminEditPagePage({
  ...
  useEffect(() => {
    async function fetchPage() {
      try {
        setLoading(true);
        setError("");
        const data = await apiClient.get<PageData>(`/pages/${id}`);
        setPageId(data._id);
        reset({
          title: data.title,
          slug: data.slug,
        ...
        if (err instanceof ApiError) {
          ...
          setError("Khong the tai thong tin trang");
        }
      } finally {
        setLoading(false);
      }
  ...
  const onSubmit = async (data: PageFormData) => {
    ...
    try {
      setSubmitting(true);
      setError("");
      await apiClient.put(`/pages/${pageId}`, data);
      router.push("/admin/pages");
    } catch (err) {
      if (err instanceof ApiError) {
        setError(err.message);
      } else {
        setError("Khong the cap nhat trang. Vui long thu lai.");
      }
    } finally {
      setSubmitting(false);
    }
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="space-y-6">
            ...
            >
              ...
              {submitting ? "Dang luu..." : "Cap nhat trang"}
            </button>
          </div>
        </div>
      </form>
